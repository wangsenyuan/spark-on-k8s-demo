ARG SPARK_IMAGE=registry.cn-hangzhou.aliyuncs.com/fesco/spark:v3.3.1
FROM ${SPARK_IMAGE}
ARG spark_uid=185
...
RUN set -ex && \
    ...
    ls -l /opt/spark/jars/zstd-jni-1.5.2-1.jar && \
    unzip /opt/spark/jars/zstd-jni-1.5.2-1.jar -d /tmp  && \
    cp /tmp/linux/amd64/libzstd-jni-1.5.2-1.so /lib && \
    mkdir -p /app/data && \
...
ADD src/main/resources/log4j.properties /opt/spark/conf/
